...
class AdminModalManager {
    ...
    // ==========================================
    // MODAL DE DOMICILIARIOS (NUEVO)
    // ==========================================
    
    async openDomiciliarioModal(mode, domiciliarioData = {}) {
        const isEdit = mode === 'edit';
        const title = isEdit ? 'Editar Domiciliario' : 'Agregar Domiciliario';
        
        // Si es edición y solo tenemos el ID, cargar datos del servidor
        if (isEdit && domiciliarioData && typeof domiciliarioData === 'number') {
            this.showLoading(true, 'Cargando datos del domiciliario...');
            try {
                const response = await fetch(`/api/domiciliarios/${domiciliarioData}`);
                if (response.ok) {
                    const data = await response.json();
                    domiciliarioData = data.domiciliario || data;
                } else {
                    this.hideLoading();
                    this.showNotification('Error al cargar los datos del domiciliario', 'error');
                    return;
                }
            } catch (error) {
                console.error('Error cargando domiciliario:', error);
                this.hideLoading();
                this.showNotification('Error de conexión al cargar el domiciliario', 'error');
                return;
            }
            this.hideLoading();
        }
        
        // Asegurar que domiciliarioData sea un objeto válido
        if (!domiciliarioData || typeof domiciliarioData !== 'object') {
            domiciliarioData = {};
        }
        ...
        modal.innerHTML = `
            <div class="modal-content modal-form">
                <div class="modal-header">
                    <h3><i class="fas fa-${isEdit ? 'edit' : 'motorcycle'}"></i> ${title}</h3>
                    <button class="modal-close" type="button">&times;</button>
                </div>
                <div class="modal-body">
                    <form class="admin-form" id="domiciliarioForm">
                        <div class="form-row">
                            <div class="form-group">
                                <label for="domiciliarioNombre">Nombre Completo *</label>
                                <input type="text" id="domiciliarioNombre" name="nombre" 
                                       value="${this.escapeHtml(domiciliarioData.nombre || '')}" 
                                       required placeholder="Ej: Juan Pérez">
                            </div>
                            <div class="form-group">
                                <label for="domiciliarioCedula">Cédula *</label>
                                <input type="text" id="domiciliarioCedula" name="cedula" 
                                       value="${this.escapeHtml(domiciliarioData.cedula || '')}" 
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                <label for="domiciliarioDisponible">Estado *</label>
                                <select id="domiciliarioDisponible" name="disponible" required>
                                    <option value="true" ${domiciliarioData.disponible === true ? 'selected' : ''}>Disponible</option>
                                    <option value="false" ${domiciliarioData.disponible === false ? 'selected' : ''}>No Disponible</option>
                                </select>
                            </div>
                        </div>
        ...
        this.bindModalEvents(modal, isEdit, domiciliarioData.id);
        this.showModal(modal);
    }
    ...
    async handleFormSubmit(form, modal, isEdit, itemId) {
        ...
        switch (this.currentSection) {
            ...
            case 'adicionales':
                ...
                break;
            case 'domiciliarios':
                data = this.extractDomiciliarioData(formData);
                break;
        }
    ...
    extractDomiciliarioData(formData) {
        return {
            nombre: formData.get('nombre'),
...
window.openAddDomiciliarioModal = function() {
    if (window.globalModalManager) {
        window.globalModalManager.openDomiciliarioModal('add');
    }
};
...
window.editDomiciliario = function(domiciliarioId) {
    if (window.globalModalManager) {
        window.globalModalManager.openDomiciliarioModal('edit', domiciliarioId);
    }
};
